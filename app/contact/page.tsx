"use client"; // Needed for the interactivity (Copy to clipboard/Form state)
...
export default function ContactPage() {
  ...
  const email = "yourname@mail.csuchico.edu"; // REPLACE THIS
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    // In a real app, you'd send this to Formspree or EmailJS
      ...
      <div className="max-w-5xl mx-auto bg-[#e5e5e5] shadow-2xl min-h-[80vh] flex flex-col md:flex-row">
        ...
        <section className="w-full md:w-5/12 p-10 bg-[#39352f] text-[#d1d1d1] flex flex-col justify-between relative overflow-hidden">
          ...
          <div>
            ...
            <p className="text-[#d67d7d] font-bold tracking-widest text-sm mb-8">OPEN TO WORK • SUMMER 2026</p>
            
            <p className="mb-8 text-sm leading-relaxed opacity-80">
              I am currently looking for internship opportunities in <b>Software Engineering</b>, <b>Game Dev</b>, or <b>Network Systems</b>. 
              <br/><br/>
              Have a question about my Cellular Automata project? Want to team up for a Hackathon? Send me a signal.
            </p>

            {/* Email Quick Copy Component */}
            <div className="mb-8">
              <label className="text-xs uppercase font-bold text-gray-500 mb-2 block">Direct Email</label>
              <button 
                onClick={handleCopy}
                className="group w-full flex items-center justify-between bg-[#2d2a26] p-4 rounded border border-gray-700 hover:border-[#d67d7d] transition-colors text-left"
              >
                <span className="font-mono text-sm md:text-base truncate">{email}</span>
          ...
          {/* Social Links */}
          <div>
            <label className="text-xs uppercase font-bold text-gray-500 mb-4 block">Connect Elsewhere</label>
            <div className="flex gap-4">
              {/* LinkedIn */}
              <a href="#" className="w-12 h-12 bg-[#2d2a26] flex items-center justify-center rounded hover:bg-[#d67d7d] hover:text-white transition-colors">
        ...
        <section className="flex-1 p-10 flex items-center justify-center">
          {formStatus === "success" ? (
            <div className="text-center animate-pulse">
              ...
              <button onClick={() => setFormStatus("idle")} className="mt-4 text-[#d67d7d] underline text-sm">Send another?</button>
            </div>
          ) : (
            <form onSubmit={handleSubmit} className="w-full max-w-md space-y-6">
              
              <div>
                <label className="block text-xs font-bold uppercase text-[#d67d7d] mb-1">Name</label>
                <input 
                  type="text" 
                  required
              ...
              <div>
                <label className="block text-xs font-bold uppercase text-[#d67d7d] mb-1">Email</label>
                <input 
                  type="email" 
                  required
              ...
              <div>
                <label className="block text-xs font-bold uppercase text-[#d67d7d] mb-1">Message</label>
                <textarea 
                  rows={4} 
                  required
              ...
              >
                {formStatus === "submitting" ? "SENDING..." : "TRANSMIT MESSAGE"}
