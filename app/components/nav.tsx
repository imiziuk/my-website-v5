...
import Link from "next/link"; // Already here, perfect.
...
export default function Nav() {
  ...
  useEffect(() => {
    const controlNavbar = () => {
      ...
      const stickAt = metrics.top + metrics.height;
      const unstickAt = metrics.top;

      if (!isFixed && currentScrollY > stickAt) {
        setIsFixed(true);
        setIsVisible(false);
        setIsInitialFix(true);
  ...
  return (
    <div
      ref={navRef}
      className="h-16 w-full relative bg-[#d1d1d1] border-b border-[#d67d7d]"
    >
      <nav
        className={`
